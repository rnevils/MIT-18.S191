...
# ╔═╡ b4cf2d8c-f206-11ea-1e93-d77d69df0b50
using LinearAlgebra
...
# ╔═╡ 1afe9d34-f1e6-11ea-3b01-9b11bf726ae9
testvar = zeros(100)

# ╔═╡ 3d93cf5e-f1e6-11ea-111c-937b33b53903
testvar[40:60] .= 1
...
# ╔═╡ 45883faa-f1e7-11ea-1672-bf1866b9a198
testvar2 = [[1,2], [3,4]]
...
# ╔═╡ ac8f0bfe-f1ea-11ea-1087-dfb2e838fce3
function readify(color)
...
function mean_colors(image)
	reds = [color.r for color in image]
	greens = [color.g for color in image]
	blues = [color.b for color in image]
...
# ╔═╡ 3e46e0aa-f1ee-11ea-10b8-21fef1215610
...
The image is unrecognisable with intensity around 2.5
"""
...
# ╔═╡ 35478760-f1ea-11ea-103d-dba557424425
size(philip)

# ╔═╡ a24c74dc-f1eb-11ea-2bf4-738b2927937c
[color.r for color in philip]

# ╔═╡ e46d922e-f1eb-11ea-050e-85afe5bf9612
readify.(philip)
...
# ╔═╡ 8042c2c2-f1fa-11ea-3b45-417b4ab68e79
testvar5 = -3:3

# ╔═╡ fb1b31de-f1fa-11ea-08ff-b59df7981e67
mean(testvar5[1:3])

# ╔═╡ 92deec1e-f1fa-11ea-1d9d-b3e5c59a871f
colored_line(example_vector)
...
function blur_1D(v, l)
	...
	for i in 1:length(v)
		blured[i] = mean([extend(v, j) for j in -1 * l + i : l + i])
	end
...
# ╔═╡ 4ca6b708-f1fe-11ea-32f2-6549cbad4fd4
@bind l_box Slider(0:1:10, show_value=true)

# ╔═╡ 8a91208e-f1fc-11ea-24d3-5d6cd6ab988d
...
function convolve_vector(v, k)
	...
	for i in 1:length(v)
		# find which elements we are multiplying
		elems = [extend(v, j) for j in -1 * l + i : l + i]
		
		# dot product with kernel
		convolved[i] = dot(elems, k)
...
# ╔═╡ 8989ab02-f210-11ea-29ae-239ef931d58f
testmat = [1 2 3
			4 5 6
			7 8 9
	]
...
function extend_mat(M::AbstractMatrix, i, j)
	...
	if i < 1
		if j < 1
			return M[1,1]
		elseif j in 1:num_columns
			return M[1, j]
		else
			return M[1,num_columns]
		end
		
	elseif i in 1:num_rows
		if j < 1
			return M[i, 1]
		elseif j in 1:num_columns
			return M[i,j]
...
# ╔═╡ e4b017e8-f218-11ea-3b3f-654b5004010d
testmat3 = [i*j for i=1:5, j=1:5]
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	# offsets to edge of kernel
	x, y = size(K) .÷ 2
	
	for i=1:num_rows, j=1:num_columns
		
		# get section of matrix to be operated on
		elems = [extend_mat(M, a, b) for a = i-x:i+x, b = j-y:j+y]

		# element-wise multiply the kernel with the section and then take sum
		convolved[i,j] = sum(elems .* K)
	end
	
	return convolved
end

# ╔═╡ b33dd896-f220-11ea-2579-49b7d89deaf1
convolve_image(testmat3, testmat)
...
function with_gaussian_blur(image)
	...
	# 2D Gaussian kernel
	G(x,y) = (1 /(2*pi)) * exp((-1*(x^2 + y^2)) / 2)
	
	# create gaussian matrix (kernel) from formula
	gaussian_kernel = [G(i,j) for i=-2:2, j = -2:2]
	
	# convolve this kernel with the image
	return convolve_image(image, gaussian_kernel)
end
