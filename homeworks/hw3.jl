...
student = (name = "Ryan", kerberos_id = "jazz")
...
# ╔═╡ b07880de-fc1b-11ea-2113-ed05065ef8f6
# tidyverse style data processing
...
# ╔═╡ 6be4680c-fc41-11ea-364c-b577f9aebc21
counts_test = Dict()

# ╔═╡ 72232b10-fc41-11ea-15a4-19c6635192dc
...
# ╔═╡ 73063816-fdd5-11ea-3b2a-fbb62023a921
test_trigrams = ngrams(split("to be or not to be that is the question", " "), 3)

# ╔═╡ a392b810-fdd5-11ea-2216-f507992fb154
test_trigrams[1][length(test_trigrams[1])]
...
function completions_cache(grams)
	...
	for ngram in grams
		...
		if haskey(cache, ngram[1:length(ngram) - 1])
			push!(cache[ngram[1:length(ngram) - 1]], ngram[length(ngram)])
		else
			cache[ngram[1:length(ngram) - 1]] = [ngram[length(ngram)]]
		end
	end
